...
int SonicCpp::sonicGetChordPitch(
    sonicStream stream)
{
...
void SonicCpp::sonicSetChordPitch(
    sonicStream stream,
    int useChordPitch)
{
...
void SonicCpp::sonicDestroyStream(
    sonicStream stream)
{
    ...
    int numChannels)
{
    ...
    if (stream->inputBuffer == NULL) {
        SonicCpp::sonicDestroyStream(stream);
        return 0;
    }
...
/* Create a sonic stream.  Return NULL only if we are out of memory and cannot
   allocate the stream. */
sonicStream SonicCpp::sonicCreateStream(
    int sampleRate,
    int numChannels)
{
...
int SonicCpp::sonicGetSampleRate(
    sonicStream stream)
{
...
/* Set the sample rate of the stream.  This will cause samples buffered in the stream to
   be lost. */
void SonicCpp::sonicSetSampleRate(
    sonicStream stream,
    int sampleRate)
{
    freeStreamBuffers(stream);
...
int SonicCpp::sonicGetNumChannels(
    sonicStream stream)
{
...
/* Set the num channels of the stream.  This will cause samples buffered in the stream to
   be lost. */
void SonicCpp::sonicSetNumChannels(
    sonicStream stream,
    int numChannels)
{
    freeStreamBuffers(stream);
...
int SonicCpp::sonicReadFloatFromStream(
    sonicStream stream,
    float *samples,
    int maxSamples)
{
...
/* Read short data out of the stream.  Sometimes no data will be available, and zero
   is returned, which is not an error condition. */
int SonicCpp::sonicReadShortFromStream(
    sonicStream stream,
    short *samples,
    int maxSamples)
{
...
/* Read unsigned char data out of the stream.  Sometimes no data will be available, and zero
   is returned, which is not an error condition. */
int SonicCpp::sonicReadUnsignedCharFromStream(
    sonicStream stream,
    unsigned char *samples,
    int maxSamples)
{
...
int SonicCpp::sonicSamplesAvailable(
    sonicStream stream)
{
...
int SonicCpp::sonicWriteFloatToStream(
    sonicStream stream,
    float *samples,
    int numSamples)
{
...
/* Simple wrapper around sonicWriteFloatToStream that does the short to float
   conversion for you. */
int SonicCpp::sonicWriteShortToStream(
    sonicStream stream,
    short *samples,
    int numSamples)
{
...
/* Simple wrapper around sonicWriteFloatToStream that does the unsigned char to float
   conversion for you. */
int SonicCpp::sonicWriteUnsignedCharToStream(
    sonicStream stream,
    unsigned char *samples,
    int numSamples)
{
...
int SonicCpp::sonicChangeFloatSpeed(
    float *samples,
    int numSamples,
    float speed,
    float pitch,
...
/* This is a non-stream oriented interface to just change the speed of a sound sample */
int SonicCpp::sonicChangeShortSpeed(
    short *samples,
    int numSamples,
    float speed,
    float pitch,
