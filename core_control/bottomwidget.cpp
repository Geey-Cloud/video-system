...
void BottomWidget::setLabCopyrightVisible(bool labCopyrightVisible)
{
    this->labCopyrightVisible = labCopyrightVisible;
}
...
void BottomWidget::setCopyright(const QString &copyright)
{
    if (this->copyright != copyright) {
        this->copyright = copyright;
        if (labCopyright) {
            labCopyright->setText(QString("版权所有: %1").arg(copyright));
        }
    }
}
...
void BottomWidget::start(int interval)
{
    ...
    if (labCopyrightVisible) {
        layout->addWidget(labCopyright);
