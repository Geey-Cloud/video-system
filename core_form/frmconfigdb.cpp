...
{
    QStringList hostInfo = ui->txtHostInfo->text().trimmed().split(":");
    if (hostInfo.count() != 2) {
        return;
    }
    ...
    AppConfig::LocalDbName = ui->txtDbName->text().trimmed();
    AppConfig::LocalHostName = hostInfo.at(0);
    AppConfig::LocalHostPort = hostInfo.at(1).toInt();
    AppConfig::LocalUserName = ui->txtUserName->text().trimmed();
    AppConfig::LocalUserPwd = ui->txtUserPwd->text().trimmed();
...
{
    {
        ...
        if (dbType == "SQLITE") {
            dbInfo.dbName = DbHelper::getDbDefaultFile(AppConfig::LocalDbName);
...
void frmConfigDb::on_btnInit_clicked()
{
    ...
    //数据库脚本名字取数据库名字/要保持一致
    QString sqlName = QString("%1/db/%2.sql").arg(QtHelper::appPath()).arg(AppConfig::LocalDbName);
    QFile file(sqlName);
