...
void MapBaiDu::addProperty(QStringList &list)
{
    ...
    //启用地图拖拽事件,默认启用(可不写)
    if (!enableDragging) {
        list << QString("  map.disableDragging();");
    }
    //启用滚轮放大缩小,默认禁用
    ...
    if (!enableDoubleClickZoom) {
        list << QString("  map.disableDoubleClickZoom();");
    }
    //启用键盘移动,默认禁用
    if (enableKeyboard) {
...
void MapBaiDu::addMarker(QStringList &list)
{
    ...
    list << QString("    } else if (iconfile == 'http://api.map.baidu.com/img/markers.png' || iconfile == './images/markers.png') {");
...
void MapBaiDu::addDevice(QStringList &list)
{
    ...
    if (count > 0) {
        ...
        list << QString("  for (var i = 0; i < count; ++i) {");
        list << QString("    var iconfile = './images/markers.png';");
