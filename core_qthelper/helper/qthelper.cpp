...
{
    return QtHelperCore::getScreenRects(available, full);
}
...
{
    return QtHelperCore::getScreenRect(available, full);
}
...
{
    return QtHelperCore::getIniValue(argv, key, dir, file);
}
...
QString QtHelper::checkPath(const QString &path)
{
    return QtHelperCore::checkPath(path);
}
...
{
    return QtHelperCore::checkFile(fileName);
}
...
{
    QtHelperNet::initLocalIPs(cbox, defaultIP, local127);
}
...
{
    return QtHelperOther::start(path, name, bin);
}
