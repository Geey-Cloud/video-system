...
{
    QStringList listDark;
    listDark << "darkblack" << "lightblue" << "lightgray" << "normalblue" << "darkgray" << "othersilvery" << "newgreen";
    bool dark = false;
    foreach (QString list, listDark) {
        if (styleName.contains(list)) {
            dark = true;
...
void GlobalStyle::getStyle(QStringList &styleNames, QStringList &styleFiles)
{
    ...
    if (files.count() == 0) {
        ...
        files << ":/qss/newblack.css" << ":/qss/newpurple.css" << ":/qss/newgreen.css";
    }
...
QString GlobalStyle::getStyle(const QString &qssFile, bool getQssColor)
{
    QString qss;
    QFile file(qssFile);
...
void GlobalStyle::setStyle(const GlobalStyle::Style &style)
{
    ...
    //设置全局样式
    QString qss = getStyle(qssFile, true);
    if (!qss.isEmpty()) {
        setStyle(qss);
    }
}
...
{
    QString qss = getStyle(qssFile, false);
    if (!qss.isEmpty()) {
