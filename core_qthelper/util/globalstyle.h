...
public:
    ...
    enum Style {
        ...
        Style_NewBlack = 18,    //新黑色样式
        Style_NewPurple = 19,   //新紫色样式
        Style_NewGreen = 20     //国网绿样式
    };

    //获取是否是加深样式
    static bool isDark1(const QString &styleName);
    static bool isDark2(const QString &styleName);
    static bool isDark3(const QString &styleName);

    //获取皮肤样式中文名称和对应的样式表文件
    static void getStyle(QStringList &styleNames, QStringList &styleFiles);
    //读取qss文件获取样式表内容
    static QString getStyle(const QString &qssFile, bool getQssColor);

    //设置全局样式
    static void setStyle(const QString &qss);
    static void setStyle(const GlobalStyle::Style &style);
