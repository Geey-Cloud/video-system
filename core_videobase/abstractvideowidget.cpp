...
AbstractVideoWidget::AbstractVideoWidget(QWidget *parent) : QWidget(parent)
{
    ...
    //关联码率采集信号用于统计显示实时码率
    connect(this, SIGNAL(sig_receiveKbps(qreal, int)), this, SLOT(receiveKbps(qreal, int)));
}
...
#ifdef openglx
    //纯音频不显示
    if (widgetPara.videoMode == VideoMode_Opengl && !onlyAudio) {
        if (hardware == "rgb") {
...
QString AbstractVideoWidget::getBannerText() const
{
    ...
    if (bannerInfo.resolution) {
        if (!onlyAudio && videoWidth > 0) {
            list << QString("分辨率: %1 x %2").arg(videoWidth).arg(videoHeight);
        }
    }
...
{
    //悬浮条可见而且启用了显示实时码率
    if (bannerWidget->isVisible() && bannerInfo.realBitRate) {
        this->kbps = kbps;
        bannerWidget->showInfo(getBannerText());
    }
}
