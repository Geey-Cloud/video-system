...
enum SaveMode {
    ...
    SaveMode_Srt = 3,           //推流到srt
    SaveMode_Udp = 4            //推流到udp
};
...
//视频编码格式
enum EncodeVideo {
    EncodeVideo_None = 0,       //不做处理
    EncodeVideo_Auto = 1,       //自动转码(源头是264就264/265就265)
    EncodeVideo_H264 = 2,       //强制用264编码
    EncodeVideo_H265 = 3        //强制用265编码
};

//音频编码格式
enum EncodeAudio {
    EncodeAudio_Auto = 0,       //自动处理(根据要保存的协议自动识别)
    EncodeAudio_Aac = 1,        //强制用aac编码
    EncodeAudio_Pcma = 2        //强制用pcma编码
};
...
enum OsdFormat {
    ...
    OsdFormat_PtsTime = 5,      //pts时间戳
    OsdFormat_Image = 6         //图片
};
...
struct OsdInfo {
    ...
    int bgPadding;              //背景边距(默认边距0是紧挨着绘制)
...
struct BannerInfo {
    ...
    bool frameRate;             //帧率
    bool mediaUrl;              //媒体地址
    bool formatName;            //封装格式
    bool videoCodecName;        //视频解码名称
    bool audioCodecName;        //音频解码名称
    bool realBitRate;           //实时码率
    bool hardware;              //硬解名称
    bool videoMode;             //视频模式
    ...
    BannerInfo() {
        ...
        mediaUrl = false;
        formatName = false;
        videoCodecName = false;
        audioCodecName = false;
        realBitRate = false;
        hardware = false;
        videoMode = false;
    }
