...
QString FilterHelper::getFilter()
{
    ...
    //高斯模糊: delogo=x=1:y=1:w=100:h=50:show=1
...
QString FilterHelper::textPadding = "10";
QString FilterHelper::getFilter(const OsdInfo &osd, bool noimage)
{
    ...
    //计算对应的坐标位置
    QString offset = FilterHelper::textPadding;
    ...
    if (isImage) {
        ...
        //linux上需要对应的ffmpeg库编译的时候指定freetype/否则不能用drawtext
        //关键字符: localtime表示本地时间 gmtime表示UTC时间
        //时间格式: https://baike.baidu.com/item/strftime/9569073 pts时间 %{pts\:hms}
        //增加毫秒: https://blog.csdn.net/weixin_33735077/article/details/92608299
        ...
        } else if (osd.format == OsdFormat_PtsTime) {
            text = "%{pts\:hms}";
        }
        ...
        //行间距/采用\n换行的时候可以看到效果
        list << QString("line_spacing=%1").arg(10);
        //文本对齐方式/ffmpeg6开始支持这个属性
        //list << QString("y_align=%1").arg("baseline");

        //背景颜色
        QColor bgColor = osd.bgColor;
        if (bgColor != Qt::transparent) {
            list << QString("box=1:boxcolor=%1@%2:boxborderw=%3").arg(bgColor.name()).arg(bgColor.alphaF()).arg(osd.bgPadding);
        }
...
QString FilterHelper::getFilter(const GraphInfo &graph, bool hardware)
{
    ...
    QString xywh = list.join(":");

    //过滤关键字用于电子放大/去水印/去水印可能有多个
    QString name = graph.name;
    if (name == "crop") {
        filter = QString("crop=%1").arg(xywh);
        return filter;
    } else if (name.startsWith("delogo")) {
        //show=1表示显示对应边框/方便查看区域是否正确
        filter = QString("delogo=%1:show=%2").arg(xywh).arg(0);
        return filter;
    }
