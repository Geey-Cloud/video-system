...
void OsdGraph::testOsdGraph(AbstractVideoWidget *widget, int fontSize, quint8 position, quint8 side)
{
    ...
    //每个方框可以指定不同的颜色
    QList<QRect> rects = QList<QRect>() << QRect(videoWidth / 30, videoHeight / 35, videoWidth / 5, videoHeight / 5) << QRect(videoWidth / 3, videoHeight / 10, videoWidth / 3, videoHeight / 2);
    QList<QColor> borderColors = QList<QColor>() << "#ff0000" << "#00ff00";

    //合并文本和区域
    QList<OsdInfo> osds;
    QList<GraphInfo> graphs;
    OsdGraph::mergeOsdGraph(texts, textColors, fontSize, rects, borderColors, borderWidth, osds, graphs, videoHeight, position, side);
