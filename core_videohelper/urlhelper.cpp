...
QString UrlHelper::getRtspUrl(const CompanyType &companyType, const QString &userName, const QString &userPwd, int channel, int streamType)
{
    QString url;
    QString head = QString("rtsp://%1:%2@[Addr]").arg(userName).arg(userPwd);
    if (companyType == CompanyType_HaiKang) {
        //?transportmode=unicast
        url = QString("/Streaming/Channels/%1%2").arg(channel + 1).arg(streamType + 1, 2, 10, QChar('0'));
    } else if (companyType == CompanyType_DaHua) {
        //&unicast=true&proto=Onvif
        url = QString("/cam/realmonitor?channel=%1&subtype=%2").arg(channel + 1).arg(streamType);
    } else if (companyType == CompanyType_YuShi) {
        url = QString("/media/video%1").arg(streamType + 1);
    } else if (companyType == CompanyType_Normal) {
        url = QString("/live?channel=%1&stream=%2").arg(channel + 1).arg(streamType);
    } else {
...
QString UrlHelper::getRtspUrl(const UrlPara &urlPara)
{
    ...
    if (urlPara.companyType == CompanyType_HaiKang) {
        ...
        //实时和回放地址格式不同
        if (urlPara.videoType == 0) {
            url = QString("%1/cam/realmonitor?%2").arg(head).arg(info);
        } else if (urlPara.videoType == 1) {
