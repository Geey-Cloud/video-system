...
void AppEvent::slot_changeVolumeWidget()
{
    emit changeVolumeWidget();
}

void AppEvent::slot_changeWeatherVisible()
{
    emit changeWeatherVisible();
}
...
{
    if (channel >= 0) {
        emit selectVideo(AppData::MediaUrls.at(channel));
    }
}

void AppEvent::slot_selectVideo(const QString &url)
{
    int channel = AppData::MediaUrls.indexOf(url);
    if (channel >= 0) {
...
void AppEvent::slot_itemPressed(QListWidgetItem *item)
{
    //按下的时候自动切换复选框状态
