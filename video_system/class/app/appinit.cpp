...
void AppInit::initOther()
{
    ...
    int height = QtHelper::deskHeight();
#else
    int width = AppConfig::FormGeometry.width();
    int height = AppConfig::FormGeometry.height();
#endif
    AppData::MapWidth = width - AppData::LeftWidth - AppData::RightWidth - 25;
    AppData::MapHeight = height - AppData::TopHeight - AppData::BottomHeight - 15;

    AppData::NvrTypes << "常规" << "海康" << "大华" << "宇视" << "其他";
    AppData::IpcTypes << "常规" << "海康" << "大华" << "宇视" << "其他";

    //目录不存在则新建
    QtHelper::checkPath(AppConfig::VideoNormalPath);
    QtHelper::checkPath(AppConfig::VideoAlarmPath);
    QtHelper::checkPath(AppConfig::ImageNormalPath);
    QtHelper::checkPath(AppConfig::ImageAlarmPath);
...
void AppInit::initWorkMode()
{
    ...
    if (AppConfig::WorkMode == 1) {
        ...
        //特定模式下强制修改功能模块配置启用禁用
        AppConfig::EnableMainOther = true;
        AppConfig::EnableMapImage = false;
        AppConfig::EnableVideoPlayWeb = false;
        AppConfig::EnableVideoPlayRtsp = false;
        ...
        AppConfig::EnableMainMap = false;
        AppConfig::EnableMapImage = false;
        AppConfig::EnableConfigUser = false;
        AppConfig::EnableConfigRecord = false;
    } else if (AppConfig::WorkMode == 4) {
        //AppConfig::SharedData = false;
        AppConfig::VolumePosition = 6;
        AppConfig::DefaultChName = "节目";
        AppConfig::DefaultIpcName = "节目#1";
        AppConfig::DefaultNvrName = "类型#1";
        AppConfig::TitleCn = QString::fromUtf8("广播电视监控报警管理平台");
        AppConfig::TitleEn = "Recording monitoring and management platform";
    } else {
