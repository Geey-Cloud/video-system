...
int DeviceHelper::initUrls()
{
    ...
    if (AppConfig::SaveUrl) {
        ...
        UrlUtil::readUrls(fileName, AppData::MediaUrls);

        //地址要求在摄像机表中存在并且处于开启状态
        for (int i = 0; i < AppData::MediaUrls.count(); ++i) {
...
void DeviceHelper::saveUrls()
{
    if (AppConfig::SaveUrl) {
        QString fileName = QtHelper::appPath() + "/config/url.txt";
        UrlUtil::writeUrls(fileName, AppData::MediaUrls);
    }
...
void DeviceHelper::changeUrls()
{
    ...
    int count = AppData::MediaUrls.count();
    for (int i = 0; i < count; ++i) {
        QString url = AppData::MediaUrls.at(i);
        if (url.isEmpty()) {
            continue;
...
void DeviceHelper::initVideoWidget(VideoWidget *videoWidget, int soundValue, bool soundMuted, bool bannerEnable, int readTimeout)
{
    ...
    bannerInfo.resolution = AppConfig::EnableResolution;
    bannerInfo.frameRate = AppConfig::EnableFrameRate;
    bannerInfo.mediaUrl = AppConfig::EnableMediaUrl;
    bannerInfo.formatName = AppConfig::EnableFormatName;
    bannerInfo.videoCodecName = AppConfig::EnableVideoCodecName;
    bannerInfo.audioCodecName = AppConfig::EnableAudioCodecName;
    bannerInfo.realBitRate = AppConfig::EnableRealBitRate;
    bannerInfo.hardware = AppConfig::EnableHardware;
    bannerInfo.videoMode = AppConfig::EnableVideoMode;
...
#endif
void DeviceHelper::preview(int serverType, const QString &hostName, int hostPort, const QStringList &mediaUrls)
{
#ifdef netpush
