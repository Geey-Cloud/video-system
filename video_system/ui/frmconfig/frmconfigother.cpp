...
void frmConfigOther::initForm()
{
    ...
    //ui->gboxMap->setVisible(false);
    //ui->gboxPath->setVisible(false);
...
void frmConfigOther::initMapConfig()
{
    ui->btnMapLocal->setChecked(AppConfig::MapLocal);
    connect(ui->btnMapLocal, SIGNAL(checkedChanged(bool)), this, SLOT(saveMapConfig()));

    for (int i = 6; i <= 20; ++i) {
        ui->cboxMapZoom->addItem(QString("%1").arg(i), i);
    }
...
void frmConfigOther::saveMapConfig()
{
    AppConfig::MapLocal = ui->btnMapLocal->getChecked();
...
void frmConfigOther::initPathConfig()
{
    ui->txtVideoNormalPath->setText(AppConfig::VideoNormalPath);
    connect(ui->txtVideoNormalPath, SIGNAL(textChanged(QString)), this, SLOT(savePathConfig()));

    ui->txtVideoAlarmPath->setText(AppConfig::VideoAlarmPath);
    connect(ui->txtVideoAlarmPath, SIGNAL(textChanged(QString)), this, SLOT(savePathConfig()));

    ui->txtImageNormalPath->setText(AppConfig::ImageNormalPath);
    connect(ui->txtImageNormalPath, SIGNAL(textChanged(QString)), this, SLOT(savePathConfig()));

    ui->txtImageAlarmPath->setText(AppConfig::ImageAlarmPath);
    connect(ui->txtImageAlarmPath, SIGNAL(textChanged(QString)), this, SLOT(savePathConfig()));

    connect(ui->btnVideoNormalPath, SIGNAL(clicked(bool)), this, SLOT(selectPath()));
    connect(ui->btnVideoAlarmPath, SIGNAL(clicked(bool)), this, SLOT(selectPath()));
    connect(ui->btnImageNormalPath, SIGNAL(clicked(bool)), this, SLOT(selectPath()));
    connect(ui->btnImageAlarmPath, SIGNAL(clicked(bool)), this, SLOT(selectPath()));
}

void frmConfigOther::savePathConfig()
{
    AppConfig::VideoNormalPath = ui->txtVideoNormalPath->text().trimmed();
    AppConfig::VideoAlarmPath = ui->txtVideoAlarmPath->text().trimmed();
    AppConfig::ImageNormalPath = ui->txtImageNormalPath->text().trimmed();
    AppConfig::ImageAlarmPath = ui->txtImageAlarmPath->text().trimmed();
    AppConfig::writeConfig();
}
...
void frmConfigOther::selectPath()
{
    //哪个按钮点击的就赋值到那个文本框
    QLineEdit *txt = ui->txtVideoNormalPath;
    QString path = AppConfig::VideoNormalPath;
    QPushButton *btn = (QPushButton *)sender();
    if (btn == ui->btnVideoNormalPath) {
        txt = ui->txtVideoNormalPath;
        path = AppConfig::VideoNormalPath;
    } else if (btn == ui->btnVideoAlarmPath) {
        txt = ui->txtVideoAlarmPath;
        path = AppConfig::VideoAlarmPath;
    } else if (btn == ui->btnImageNormalPath) {
        txt = ui->txtImageNormalPath;
        path = AppConfig::ImageNormalPath;
    } else if (btn == ui->btnImageAlarmPath) {
        txt = ui->txtImageAlarmPath;
        path = AppConfig::ImageAlarmPath;
    }

    path = QtHelper::getExistingDirectory(path);
    if (!path.isEmpty()) {
        txt->setText(path);
    }
}
