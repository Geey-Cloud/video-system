...
void frmConfigPlus::on_btnAddDevice_clicked()
{
    QString audioDevice = ui->cboxAudioDevice->currentText();
    QString videoDevice = ui->cboxVideoDevice->currentText();
    //如果设备参数那边有下拉选择则还需要添加分辨率帧率等参数
    QString option = ui->cboxDeviceOption->currentText();
    QString url = DeviceInfoHelper::getOptionUrl(audioDevice, videoDevice, option);
    if (url.isEmpty()) {
        return;
    }

    QStringList addrs = QStringList() << url;
    emit addPlus(addrs, addrs);
}

void frmConfigPlus::on_btnGetDevice_clicked()
{
    DeviceInfoHelper::initInputDevices(ui->cboxAudioDevice, ui->cboxVideoDevice);
}

void frmConfigPlus::on_btnGetOption_clicked()
{
    DeviceInfoHelper::initInputOptions(ui->cboxAudioDevice, ui->cboxVideoDevice, ui->cboxDeviceOption);
}

void frmConfigPlus::on_cboxVideoDevice_currentIndexChanged(int index)
{
    ui->cboxDeviceOption->clear();
}
...
void frmConfigPlus::on_cboxPlusType_currentIndexChanged(int index)
{
    ...
    if (index == 0) {
        ...
        rtspMain = UrlHelper::getRtspUrl(companyType, "admin", "12345", 0, 0);
        rtspSub = UrlHelper::getRtspUrl(companyType, "admin", "12345", 0, 1);
    } else {
        rtspMain = UrlHelper::getRtspUrl(CompanyType_Normal, "admin", "12345", 0, 0);
        rtspSub = UrlHelper::getRtspUrl(CompanyType_Normal, "admin", "12345", 0, 1);
    }
