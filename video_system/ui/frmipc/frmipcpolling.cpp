...
void frmIpcPolling::polling()
{
    ...
    for (int i = 0; i < list.count(); ++i) {
        ...
        VideoWidget *videoWidget = AppData::videoPanel->videoWidgets.at(i);
        videoWidget->open(url);
        //videoWidget->open(url, 1000 + i * 100);
        AppData::MediaUrls[i] = url;
...
void frmIpcPolling::on_btnPollStart_clicked()
{
    ...
    if (!AppConfig::Polling) {
        //切换到对应的轮询通道画面数
        AppConfig::LayoutType = QString("1_%1").arg(AppConfig::PollType);
        //1通道需要特殊处理
        ...
        AppData::videoPanel->videoBox->change_layout(AppConfig::PollType, 0);
