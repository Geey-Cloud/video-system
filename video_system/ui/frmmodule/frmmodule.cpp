...
void frmModule::addWidget()
{
    ...
    //要调整模块位置，必须删除bin/layout下的文件才可以调整
    if (AppConfig::WorkMode == 1) {
        //添加右侧窗体
        addWidget(2, 1);
        addWidget(3, 1);
...
void frmModule::raiseWidget(int index)
{
    if (index < dockWidgets.count()) {
        dockWidgets.at(index)->raise();
    }
}

void frmModule::hideWidget(int index)
{
    if (index < dockWidgets.count()) {
        dockWidgets.at(index)->hide();
    }
}
