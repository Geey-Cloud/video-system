...
void frmVideoPanelTool::buttonClicked()
{
    ...
    if (btn == ui->btnChannel1) {
        AppData::videoPanel->videoBox->change_layout_1(0);
    } else if (btn == ui->btnChannel4) {
        AppData::videoPanel->videoBox->change_layout_4(0);
    } else if (btn == ui->btnChannel6) {
        AppData::videoPanel->videoBox->change_layout_6(0);
    } else if (btn == ui->btnChannel8) {
        AppData::videoPanel->videoBox->change_layout_8(0);
    } else if (btn == ui->btnChannel9) {
        AppData::videoPanel->videoBox->change_layout_9(0);
    } else if (btn == ui->btnChannel13) {
        AppData::videoPanel->videoBox->change_layout_13(0);
    } else if (btn == ui->btnChannel16) {
        AppData::videoPanel->videoBox->change_layout_16(0);
    } else if (btn == ui->btnChannel25) {
        AppData::videoPanel->videoBox->change_layout_25(0);
    } else if (btn == ui->btnChannel36) {
        AppData::videoPanel->videoBox->change_layout_36(0);
    } else if (btn == ui->btnChannel64) {
        AppData::videoPanel->videoBox->change_layout_64(0);
    } else if (btn == ui->btnSnap) {
        AppData::videoPanel->snapOne();
    } else if (btn == ui->btnSound) {
        //弹出音量条进行控制/先计算音量按钮的全局坐标
    ...
    if (channel >= 1) {
        if (btn == ui->btnChannel1) {
            ...
            AppConfig::LayoutType = "1_1x1";
        } else {
            AppConfig::LayoutType = QString("1_%1").arg(channel);
        }
...
void frmVideoPanelTool::changeLayout(int channel)
{
    //从通道按钮组中找到对应的按钮设置选中
...
void frmVideoPanelTool::changeVideo(int channel1, int channel2)
{
    ...
    //重新保存url
    AppData::MediaUrls[channel1] = widget2->getVideoPara().mediaUrl;
    AppData::MediaUrls[channel2] = widget1->getVideoPara().mediaUrl;
    DeviceHelper::saveUrls();
...
int frmVideoPanelTool::getFreeChannel()
{
    ...
    int index = -1;
    for (int i = 0; i < AppConfig::VideoCount; ++i) {
        QString url = AppData::MediaUrls.at(i);
        if (!url.isEmpty()) {
            index = i;
        }
    }
    ...
    if (isNvr) {
        ...
        for (int i = 0; i < DbData::IpcInfo_Count; ++i) {
            if (DbData::IpcInfo_NvrName.at(i) == text) {
                ...
                AppData::MediaUrls[index] = rtspAddr;
