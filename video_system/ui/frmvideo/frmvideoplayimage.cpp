...
void frmVideoPlayImage::initForm()
{
    ...
    //关联按下行自动切换选中状态
    connect(ui->listWidget, SIGNAL(itemPressed(QListWidgetItem *)), AppEvent::Instance(), SLOT(slot_itemPressed(QListWidgetItem *)));
}
...
void frmVideoPlayImage::on_btnSelect_clicked()
{
    ...
    if (ui->cboxType->currentText() == "存储图片") {
        filePath = AppConfig::ImageNormalPath;
    } else {
        filePath = AppConfig::ImageAlarmPath;
    }
